...
We can also pass in other graphical parameters. Let's add a title (`main=`), increase the y-axis limit (`ylim=`), reduce the point size to 50% (`cex=`), and reduce the font size of the axis labels to 80% (`cex.axis=`):

```{r}
manhattan(gwasResults, main="Manhattan Plot", ylim=c(0,12), cex=0.5, cex.axis=0.8)
```

Let's change the colors and remove the suggestive and genome-wide significance lines:
